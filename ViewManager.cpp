...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	glfwSetScrollCallback(window, &ViewManager::MouseScroll);
...
{
	// Active when mouse input is first received. Needed to acuretly depict future movements.
	if (gFirstMouse) {
	...
	// calculate the x and y offset of each movement 
	float xOffset = xMousePos - gLastX;
	float yOffset = gLastY - yMousePos; // reversed due to y going from bottom to top

	// set current positions to the last know position variables
	gLastX = xMousePos;
	gLastY = yMousePos;

	// move the 3D camera accordingly
 ...
 ***********************************************************/
void ViewManager::MouseScroll(GLFWwindow* window, double xoffset, double yoffset) {
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// process change in perspective
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS) { // P for perspective
		bOrthographicProjection = false;
	}

	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS) { // O for orthographic
